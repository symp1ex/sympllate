...
func TestCleanupChangesOnlyConfirmedBoxesAndPreservesAlpha(t *testing.T) {
	...
	renderer, err := NewRenderer(t.TempDir(), DefaultRenderConfig())
	if err != nil {
		t.Fatal(err)
	}
	document := RenderDocument{Blocks: []RenderBlock{{CleanupBox: ocr.OCRBox{X: 5, Y: 6, Width: 4, Height: 3}, Background: newRenderColor(color.NRGBA{R: 200, G: 210, B: 220, A: 123})}}}
	cleaned, err := renderer.Clean(context.Background(), source, document)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPrepareSkipsUntranslatedBlockAndDrawsCyrillic(t *testing.T) {
	...
	renderer, err := NewRenderer(directory, DefaultRenderConfig())
	if err != nil {
		t.Fatal(err)
	}
...
func TestPNGCleanupKeepsPixelsOutsideBoxByteEquivalentAfterDecode(t *testing.T) {
	...
	cleaned, err := renderer.Clean(context.Background(), source, RenderDocument{Blocks: []RenderBlock{{CleanupBox: ocr.OCRBox{X: 2, Y: 2, Width: 2, Height: 2}, Background: newRenderColor(color.NRGBA{R: 200, A: 255})}}})
	if err != nil {
		t.Fatal(err)
	}
